...
#include "AuthlibInjectorDownloadTask.h"
...
void AuthlibInjectorDownloadTask::executeTask()
{
    setStatus(tr("Checking authlib-injector version..."));

    // Check if we already have a valid cached version
    QFileInfo fileInfo(m_jarPath);
    if (fileInfo.exists() && fileInfo.size() > 100 * 1024) {
        // File exists and is larger than 100KB, consider it valid
        emitSucceeded();
        return;
    }

    // Need to download - first try BMCLAPI, then fall back to official
...
void AuthlibInjectorDownloadTask::downloadInfoFinished()
{
...
void AuthlibInjectorDownloadTask::startJarDownload(const QString& url, const QString& checksum)
{
    ...
    connect(m_jarReply, &QNetworkReply::finished, this, &AuthlibInjectorDownloadTask::downloadJarFinished);
    connect(m_jarReply, &QNetworkReply::downloadProgress, this, [this](qint64 received, qint64 total) {
        m_totalSize = total;
        m_downloadedSize = received;
        if (total > 0) {
            setProgress(received, total);
        }
        setDetails(tr("%1 / %2").arg(received / 1024).arg(total / 1024) + " KB");
    });

    m_expectedChecksum = checksum;
}

void AuthlibInjectorDownloadTask::downloadJarFinished()
{
