...
#include "Nide8AuthDownloadTask.h"
...
void Nide8AuthDownloadTask::executeTask()
{
    setStatus(tr("Checking nide8auth.jar..."));

    // Check if we already have a valid cached version
    QFileInfo fileInfo(m_jarPath);
    if (fileInfo.exists() && fileInfo.size() > 50 * 1024) {
        // File exists and is larger than 50KB, consider it valid
        emitSucceeded();
        return;
    }
    ...
    connect(m_jarReply, &QNetworkReply::finished, this, &Nide8AuthDownloadTask::downloadJarFinished);
...
void Nide8AuthDownloadTask::downloadJarFinished()
{
